"""
2D field trace plotter — single-robot focus mode.
...
VOTE_THRESHOLD     = 3
TARGET_NUMBER      = "2073"
ALL_ROBOT_NUMBERS  = ["2910","1323","4272","2073","4414","1690"]    # e.g. ["1690","2073","118","3538","1678","5940"]
...
# --------------------------------------------------------------------------
# Phase 1: seek to field view, click 4 corners
# --------------------------------------------------------------------------
corners = []
...
def on_corner_click(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN and len(corners) < 4:
        ...
        if len(corners) == 4:
            print("All 4 corners set — press Enter to start tracking.")
    ...
    if not (result.boxes and result.boxes.is_track):
        ...
        if target_track_id not in track_ids:
            ...
            if ref and track_ids:
                # Only consider tracks not already confirmed as a different robot
        ...
        if frame_count % OCR_EVERY_N_FRAMES == 0:
            for box_xyxy, tid in zip(boxes_xyxy, track_ids):
                ...
                for t in reader.readtext(crop_up, detail=0, allowlist='0123456789'):
                    t = t.strip()
                    if not t:
                        continue
                    if ALL_ROBOT_NUMBERS:
                        ...
                        # No known list: only accept exact or partial match to target
